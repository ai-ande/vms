...
- **homeassistant-vm/ha-backup-mirror** — mirrors Home Assistant `*.tar` backups
  from the SMB landing zone (`/Users/Shared/ha-backups`) to **Proton Drive** via
  the official Proton Drive CLI. It lives under `homeassistant-vm/` because it
  backs up that VM's data, but it is still an independently-installable module
  (its own `install.sh`/plist). Runs as a **gui LaunchAgent for akaplan** (needs
  akaplan's GUI session for the login Keychain). Success requires the backup to
  be verified present in Proton Drive — not merely copied.
